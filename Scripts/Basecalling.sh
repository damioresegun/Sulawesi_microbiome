...
Save_OUT=$2   # output folder for the basecalling
...
#guppy basecaller
gupbase=`$ONT/guppy_basecaller -i ${Raw_IN} --save_path ${Base_OUT} --flowcell $FLOWCELL --kit $KIT --disable_pings -r -v -q 0 -x auto --trim_adapters --gpu_runners_per_device 8 --chunks_per_runner 786 2>&1 | tee $Log_Ouut/Basecalling.txt`
