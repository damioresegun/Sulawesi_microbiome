...
def DNA_align(ready_path):
    for isolate in DNA_ISOLATE:
            ...
            statss = os.path.join(stats_dir, "Alignment_Vs_Reference", isolate)
            if os.path.exists(statss):
                pass
            else:
                os.makedirs(statss)
            # run the alignment of DNA against the reference
            hs_reads = align(isolate, file, temp_align_out, aligned_out, THREADS, statss, REFERENCE)
            # Run de-duplication
...
def cDNA_align(ready_path):
    for isolate in CDNA_ISOLATE:
            # set the right filtered/demultiplexed file
            file = ready_path + "/" + isolate + ".fastq.gz"
            ...
            hfFileOt = aligned_out + "/" + isolate + "VsRef_unmapped.fastq"
            ...
            hfRnm = aligned_out + "/" + isolate + "VsRef_unmapped_renamed.fastq"
...
def cdna_Kraken(isolate, seq, BRAKLENGTH):
    ...
    krakOut = os.path.join(OUT_DIR, "Kraken", combs)
    ...
    cKrak = (KRAK, "--db", KRAKDB, toAlgn, "--threads", str(THREADS),
            ...
            "--minimum-hit-groups", str(KRAK_THRESH), "--report-minimizer-data --confidence 0.1")
    runCkrak = ' '.join(cKrak)
    print(runCkrak)
    ...
    cutKrak = ("cut -f1-3,6-8", krakOut + "/FULLreport.txt", ">", krakOut + "/ClassicFullReport")
    runCutKrak = ' '.join(cutKrak)
    subprocess.call(runCutKrak, shell=True)
    # carry out bracken
    Brak = (BRAK, "-d", KRAKDB, "-i", krakOut + "/ClassicFullReport", "-o", 
            krakOut + "/bracken_report.txt", "-t", str(BRAKTHRESH), "-w",
            krakOut + "/bracken_KrakenReport.txt", "-r", str(BRAKLENGTH))
    runBrak = ' '.join(Brak)
    print(runBrak)
    #
...
if __name__ == '__main__':
    logger = logging.getLogger('NanoMetaPipe_assemblyApproach.py: %s' % time.asctime())
    ...
    for file in os.listdir(dem_dir):
        zip_dem = os.path.join(dem_dir, file)
    ...
    ###########################################################################################
    for isolate in ISOLATES:
        toAlgn = aligned_out + "/" + isolate + "VsRef_unmapped_renamed.fastq"
    ...
    if SEQ_TYP == "cdna":
        for isolate in CDNA_ISOLATE:
            ...
            ClasassemOut = os.path.join(OUT_DIR, "DeNoVo_Assembly", isolate, "Classified")
            UncClassemOut = os.path.join(OUT_DIR, "DeNoVo_Assembly", isolate, "Unclassified")
            ...
            ClasassemSt = os.path.join(stats_dir, "DeNoVo_Assembly", isolate, "Classified")
            UncClassemSt = os.path.join(stats_dir, "DeNoVo_Assembly", isolate, "Unclassified")
            ...
            run_AssemStats(isolate, ClassemFile, ClassemO)
            run_AssemStats(isolate, UnclassemFile, UnclassemO)
            print('The generated assembly will be copied to a new folder')
            ...
            ClacoAssemFile = os.path.join(assemS, (isolate + "_classified.fasta"))
            UnclacoAssemFile = os.path.join(assemS, (isolate + "_unclassified.fasta"))
